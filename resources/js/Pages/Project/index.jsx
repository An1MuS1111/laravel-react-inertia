...
export default function Index({ auth, projects, queryParams = null, success }) {
    ...
    const onKeyPress = (name, e) => {
        if (e.key !== 'Enter') return; searchFieldChanged(name, e.target.value)
    }
                                ...
                                <tbody>
                                    {projects.data.map((project) => (
                                        <tr className='bg-white border-b dark:bg-gray-800 dark:border-gray-700' key={project.id}>
                                            ...
                                            <td className='px-3 py-2'><img src={project.image_path} alt="" style={{ width: 50, height: 50 }} /></td>
                                            <td className='px-3 py-2'>{project.name}</td>
                                            <td className='px-3 py-2'>

                                                <span className={"px-2 py-1 rounded text-white " +
                                                    PROJECT_STATUS_CLASS_MAP[project.status]} >{PROJECT_STATUS_TEXT_MAP[project.status]}</span>
